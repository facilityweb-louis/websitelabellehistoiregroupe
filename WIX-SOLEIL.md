...
Si rien n'est posé côté Wix, rien ne casse : le composant garde sa hauteur
d'écran et la page défile à l'intérieur, comme avant.
...
Le fragment d'origine, auquel on n'ajoute que le relais. Le `height: 100%` est
conservé : le cadre suivra la hauteur du composant dès que Wix la lui aura
donnée.
...
Les règles `.embedded` en tête de `soleil.html` coupent la boucle en figeant
`--frame-h`, la hauteur du cadre relevée avant tout redimensionnement — soit
exactement ce que `svh` donnait. **Tout nouvel élément en `vh`, `svh` ou `dvh`
doit recevoir sa règle `.embedded`.**

Deux tentatives ont échoué avant celle-ci, de la même façon : la hauteur du
cadre était relevée dans l'en-tête, avant la mise en page. Elle valait alors
zéro, les sections qui s'y adossent s'effondraient, et toute la page se repliait
en son milieu. D'où deux garde-fous, à ne pas retirer :

- la hauteur est lue en fin de corps, une fois la mise en page faite ;
- elle est rejetée hors de l'intervalle 400–2000 px, et la page garde alors ses
  `svh`, c'est-à-dire sa mise en page d'aujourd'hui. Aucun message n'est émis
  tant qu'une hauteur crédible n'a pas été adoptée.
